...
The first plot above shows a histogram of the target variable, shares. We can see the number of shares is heavily skewed. The histogram of log shares looks better.  
...
temp <- train %>%
  ...
  summarize(avg_shares = mean(shares)) %>%                      #Creating summary stat
...
#Creating a bar chart of average shares per channel
h <- ggplot(temp, aes(x = channel, y = avg_shares))
h + geom_bar(aes(fill = channel), stat = 'identity', show.legend = FALSE) +
  scale_x_discrete(labels = c('Business', 'Entertainment', 'Lifestyle', 
                              'Social Media', 'Technology', 'World' )) +
  scale_fill_brewer(palette = 'Set2') +
  labs(title = 'Average Shares by Data Channel', x = 'Channel', y = 'Average Shares')

```
  
The plot above shows of the average shares per channel for the selected day.  
